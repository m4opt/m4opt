"""
Python conversion of the MATLAB script `refraction_index.m` by Eran Ofek,
available at:
https://github.com/EranOfek/AstroPack

This module computes the refractive index and transmission for optical
materials as a function of wavelength.
"""

import astropy.units as u
import numpy as np


def get_refraction_index(material="si02_suprasil_2a", temperature=20):
    """
    Compute refractive index and transmission for a given material.
    See module docstring for context and references.

    Parameters
    ----------
    material : str, optional
        Supported options:
        - 'sio2', 'silica' : fused silica (Sellmeier equation)
        - 'si02_suprasil_2a' : Suprasil 2A (tabulated data)
        Default: 'si02_suprasil_2a'.

    temperature : float, optional
        Temperature in Celsius. Only 20°C supported.

    Returns
    -------
    L : np.ndarray
        Wavelengths [Å].
    n : np.ndarray
        Refractive index.
    t : np.ndarray or None
        Transmission per 1 cm (only for Suprasil 2A), or None.

    Raises
    ------
    ValueError
        For unsupported material or temperature.
    """

    material = material.lower()
    if material in ["sio2", "silica"]:
        # Sellmeier equation for fused silica
        L = np.arange(0.21, 6.71, 0.01)  # in microns
        n = np.sqrt(
            1
            + 0.6961663 * L**2 / (L**2 - 0.0684043**2)
            + 0.4079426 * L**2 / (L**2 - 0.1162414**2)
            + 0.8974794 * L**2 / (L**2 - 9.896161**2)
        )
        L = L * 1e4  # convert µm to Å
        t = None

    elif material == "si02_suprasil_2a":
        # Tabulated data
        n_data = np.array(
            [
                [1862, 1.5732],
                [1963.4, 1.5563],
                [2064.8, 1.543],
                [2166.1, 1.5322],
                [2267.5, 1.5233],
                [2368.9, 1.5159],
                [2470.3, 1.5096],
                [2571.7, 1.5043],
                [2673, 1.4996],
                [2774.4, 1.4956],
                [2875.8, 1.492],
                [2977.2, 1.4889],
                [3078.6, 1.4861],
                [3179.9, 1.4837],
                [3281.3, 1.4814],
                [3382.7, 1.4794],
                [3484.1, 1.4776],
                [3585.5, 1.4759],
                [3686.8, 1.4744],
                [3788.2, 1.4731],
                [3889.6, 1.4718],
                [3991, 1.4706],
                [4092.4, 1.4696],
                [4193.7, 1.4686],
                [4295.1, 1.4676],
                [4396.5, 1.4668],
                [4497.9, 1.466],
                [4599.3, 1.4652],
                [4700.6, 1.4645],
                [4802, 1.4639],
                [4903.4, 1.4633],
                [5004.8, 1.4627],
                [5106.2, 1.4622],
                [5207.5, 1.4616],
                [5308.9, 1.4612],
                [5410.3, 1.4607],
                [5511.7, 1.4603],
                [5613.1, 1.4598],
                [5714.4, 1.4594],
                [5815.8, 1.4591],
                [5917.2, 1.4587],
                [6018.6, 1.4584],
                [6120, 1.458],
                [6221.3, 1.4577],
                [6322.7, 1.4574],
                [6424.1, 1.4571],
                [6525.5, 1.4569],
                [6626.9, 1.4566],
                [6728.2, 1.4563],
                [6829.6, 1.4561],
                [6931, 1.4558],
                [7032.4, 1.4556],
                [7133.8, 1.4554],
                [7235.1, 1.4552],
                [7336.5, 1.455],
                [7437.9, 1.4547],
                [7539.3, 1.4545],
                [7640.7, 1.4544],
                [7742, 1.4542],
                [7843.4, 1.454],
                [7944.8, 1.4538],
                [8046.2, 1.4536],
                [8147.6, 1.4535],
                [8248.9, 1.4533],
                [8350.3, 1.4531],
                [8451.7, 1.453],
                [8553.1, 1.4528],
                [8654.5, 1.4526],
                [8755.8, 1.4525],
                [8857.2, 1.4523],
                [8958.6, 1.4522],
                [9060, 1.4521],
                [9161.4, 1.4519],
                [9262.7, 1.4518],
                [9364.1, 1.4516],
                [9465.5, 1.4515],
                [9566.9, 1.4514],
                [9668.3, 1.4512],
                [9769.6, 1.4511],
                [9871, 1.451],
                [9972.4, 1.4508],
                [10074, 1.4507],
                [10175, 1.4506],
                [10277, 1.4505],
                [10378, 1.4503],
                [10479, 1.4502],
                [10581, 1.4501],
                [10682, 1.45],
                [10783, 1.4498],
                [10885, 1.4497],
                [10986, 1.4496],
                [11088, 1.4495],
                [11189, 1.4494],
                [11290, 1.4492],
                [11392, 1.4491],
                [11493, 1.449],
                [11594, 1.4489],
                [11696, 1.4488],
                [11797, 1.4487],
                [11899, 1.4485],
                [12000, 1.4484],
            ]
        )
        L = n_data[:, 0]
        n = n_data[:, 1]

        t_data = np.array(
            [
                [1400, 0],
                [1450, 0],
                [1500, 0],
                [1550, 0],
                [1600, 0.000239],
                [1650, 0.0927],
                [1700, 0.687],
                [1750, 0.886],
                [1800, 0.94],
                [1850, 0.968],
                [1900, 0.985],
                [1950, 0.99],
                [2000, 0.993],
                [2050, 0.996],
                [2100, 0.998],
                [2150, 0.999],
                [2200, 0.999],
                [2250, 0.999],
                [2300, 0.999],
                [2350, 0.999],
                [2400, 1],
                [2450, 1],
                [2500, 1],
                [2550, 1],
                [2600, 1],
                [2650, 1],
                [2700, 1],
                [2750, 1],
                [2800, 1],
                [2850, 1],
                [2900, 1],
                [2950, 1],
                [3000, 1],
                [4000, 1],
                [5000, 1],
                [6000, 1],
                [7000, 1],
                [8000, 1],
                [9000, 1],
                [10000, 1],
                [11000, 1],
                [12000, 1],
                [13000, 1],
                [13800, 0.902],
                [14000, 0.951],
                [15000, 1],
                [16000, 1],
                [17000, 1],
                [18000, 0.999],
                [19000, 0.996],
                [20000, 0.995],
                [20300, 0.992],
                [20500, 0.988],
                [20800, 0.986],
                [21000, 0.986],
                [21300, 0.981],
                [21500, 0.964],
                [21800, 0.933],
                [22000, 0.765],
                [22100, 0.681],
                [22300, 0.723],
                [22500, 0.767],
                [22600, 0.779],
                [22800, 0.844],
                [23000, 0.908],
                [23300, 0.94],
                [23500, 0.953],
                [23800, 0.955],
                [24000, 0.947],
                [24300, 0.927],
                [24500, 0.903],
                [24800, 0.88],
                [25000, 0.842],
                [25300, 0.786],
                [25500, 0.717],
                [25800, 0.655],
                [26000, 0.605],
                [26300, 0.556],
                [26500, 0.458],
                [26800, 0.18],
                [27000, 1.05e-5],
                [27200, 1e-8],
                [27300, 1e-8],
                [27500, 2.9e-7],
                [27800, 3.41e-5],
                [28000, 0.00171],
                [28300, 0.0321],
                [28500, 0.162],
                [28800, 0.345],
                [29000, 0.494],
                [29300, 0.599],
                [29500, 0.67],
                [29800, 0.718],
                [30000, 0.751],
                [30300, 0.776],
                [30500, 0.794],
                [30800, 0.808],
                [31000, 0.814],
                [31300, 0.819],
                [50000, 0],
            ]
        )
        t = t_data

    else:
        raise ValueError("Unknown material option")

    if temperature != 20:
        raise ValueError("Only 20°C data is implemented")

    return L * u.Angstrom, n, t / u.cm
